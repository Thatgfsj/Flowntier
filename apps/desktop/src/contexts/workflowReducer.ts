...
function reduceEvent(state: RootState, event: WfEvent): RootState {
  ...
  if (event.kind === 'done') {
    ...
    const aborted =
      status.startsWith('ABORTED') || status.startsWith('FAILED');
  ...
  // ── transition (the runtime only emits `transition`; the
  //    historical `phase_transition` kind was removed in v0.4.22)
  if (
...
function upsertTask(tasks: TaskRow[], fresh: TaskRow): TaskRow[] {
  ...
  if (existing) {
    ...
    // `undefined`.
    const merged: TaskRow = fresh.summary !== undefined
