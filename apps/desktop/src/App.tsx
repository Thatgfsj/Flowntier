...
function DriftBanner({ sidecar, minCompatible, onDismiss }: DriftBannerProps) {
  ...
  return (
    ...
    >
      <span>{t("drift.message", { sidecar, expected: minCompatible })}</span>
      <button
        type="button"
        onClick={onDismiss}
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (cancelled) return;
        const isFirst = !r || r.v === null || r.v === "true" || r.v === true;
        setFirstRun(isFirst);
      } catch (e) {
  ...
  if (workdir === null && !workdirSkipped) {
    return (
      <WorkdirSetup
        ...
        onSkip={async () => {
          try {
            await invoke("kv_set", { key: "workdir_skipped", value: true });
          } catch {}
          try {
            await invoke("clear_workdir");
          } catch {}
  ...
  function WorkbenchApp({ workdir }: { workdir: string | null }) {
    ...
    const startRealWorkflow = async (text: string) => {
      ...
      try {
        ...
        // Uses stateRef to read the live state and avoid React stale-closure traps.
        ...
        while (Date.now() < deadline) {
          ...
          try {
            ...
            const isTerminal =
              st === "DONE" ||
              (typeof st === "string" && (st.startsWith("ABORTED") || st.startsWith("FAILED")));
            const isDelivery = phase === "delivery" || phase === "8-delivery";

            if (wf && (isTerminal || isDelivery) && wf.summary) {
        ...
        // If timed out and still busy, mark repair
        if (!stateRef.current.completed && stateRef.current.busy) {
          const err = stateRef.current.workflowError;
          if (err !== null) {
            dispatch({ type: "SET_REVIEW_VERDICT", verdict: { verdict: "REPAIR", summary: err } });
          } else {
            dispatch({
              type: "SET_REVIEW_VERDICT",
              verdict: { verdict: "REPAIR", summary: t("workflow.verdict.timeout") },
            });
          }
          dispatch({ type: "SET_BUSY", value: false });
          dispatch({ type: "SET_COMPLETED", value: true });
    ...
    return (
      <div className="flex h-screen flex-col">
        ...
        <div className="flex flex-1 min-h-[280px] overflow-hidden">
          <aside
            className="w-[260px] shrink-0 overflow-y-auto border-r border-white/8 bg-surface-1/95 p-3 backdrop-blur-sm"
          ...
          <main
            ...
            className="flex-1 overflow-y-auto bg-surface-0/60 p-4 space-y-3"
          >
            <PhaseTimeline
              steps={PHASES.map((p) => ({
                name: p.name,
              ...
              <Card className="border-status-error/60 bg-status-error/10">
                ...
                <p className="mt-2 text-xs text-text-secondary">
                  {t("workflow.error.hint", {
                    defaultValue:
                      "Most common cause: the API key stored for this role has expired or is wrong. Open Settings → Providers → MiMo and re-save the key.",
                  })}
                </p>
              </Card>
            )}
          ...
          <aside
            className="w-[360px] shrink-0 overflow-y-auto border-l border-white/8 bg-surface-1/95 p-3.5 backdrop-blur-sm"
        ...
        <div
          className={`relative flex shrink-0 border-t border-border transition-[height] ${
            chatOpen ? "h-[320px] max-h-[35vh] min-h-[180px]" : "h-9"
          }`}
        >
          {chatOpen ? (
                ...
                activeAgent={(() => {
                  ...
                  const headRoles: ReadonlyArray<{
                  ...
                  }> = [
                    {
                      role: "chief",
                      i18nKey: "chief",
                      phases: ["requirement", "plan", "dispatch", "repair", "delivery"],
                    },
                    {
                      role: "critic-a",
                      i18nKey: "criticA",
                      phases: ["plan-review", "final-review"],
                    },
                    {
                      role: "critic-b",
                      i18nKey: "criticB",
            ...
            >
              ...
              <span>
                {t("chatZone.subtitle", { defaultValue: "由设置 → 角色 → 模型 分配驱动" })}
              </span>
            </button>
          )}
        </div>
...
function TaskProgressPanel({ summary, phases, phaseStates }: TaskProgressPanelProps) {
  ...
  const activeIdx = phases.findIndex(
    (p) => p && (p.name === activePhaseName || p.name === summary.phase),
  );
    ...
    <Card className="!p-3">
      <div className="mb-2 flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="break-words text-sm text-text-primary" title={req}>
      ...
      <div className="mb-2 flex items-center gap-1">
        {phases.map((p, i) => {
          ...
          const cls =
            s === "done" ? "bg-status-done" : isActive ? "bg-chief flt-anim-pulse" : "bg-surface-3";
          return (
            <div
              key={p.name}
      ...
      <div className="flex items-center justify-between text-[10px] text-text-secondary">
        ...
        {tasksTotal > 0 && (
          <span>{t("workbench.taskProgress", { done: tasksDone, total: tasksTotal })}</span>
        )}
        {activePhase && (
          <span className="ml-auto font-mono">
            {t("workbench.phaseName", {
              name: phaseNameLabel[activePhase.name] ?? activePhase.name,
            })}
          </span>
        )}
      </div>
