...
export function PerTaskConsole({ taskId, events, className }: PerTaskConsoleProps) {
  ...
  if (taskEvents.length === 0) {
    return (
      <div className={`text-xs text-text-secondary ${className ?? ""}`}>{t("perTask.empty")}</div>
    );
  }
          ...
          if (e.kind === "console") {
            return (
              <div key={i} className="flex gap-2 py-0.5">
                ...
                <span className={`shrink-0 ${LEVEL_COLORS[e.level]}`}>{LEVEL_LABELS[e.level]}</span>
                <span className="shrink-0 text-text-secondary">{agentToLabel(t, e.agent_id)}</span>
                <span className="min-w-0 flex-1 truncate text-primary">{e.message}</span>
              </div>
            );
          }
