  ...
  return (
    <div className="relative">
      <button
        ...
        aria-expanded={open}
        title={
          rows.length === 0 ? "no recent errors" : `${errorCount} error(s), ${warnCount} warning(s)`
        }
        ...
        {rows.length > 0 && (
          <span
            className={
              ...
              (errorCount > 0 ? "bg-status-failed text-black" : "bg-status-active text-black")
            }
          >
            {rows.length}
          </span>
      ...
      {open && (
        ...
        >
          ...
          {rows.length === 0 && (
            <div className="px-2 py-1 text-[11px] text-text-secondary">暂无错误</div>
          )}
          <ul className="max-h-[60vh] space-y-1 overflow-y-auto">
            {rows.map((r, i) => (
              ...
              >
                ...
                {r.detail && (
                  <div className="mt-0.5 text-[10px] text-text-secondary">{r.detail}</div>
                )}
              </li>
            ))}
          </ul>
